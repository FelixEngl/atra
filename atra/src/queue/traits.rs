...
#[allow(dead_code)]
pub trait AgingQueue<T: AgingQueueElement + Serialize + DeserializeOwned + Debug>:
    RawAgingQueue
{
    /// Enqueue a value of type [T].
    #[inline]
    async fn enqueue(&self, entry: T) -> Result<(), QueueError> {
        unsafe { self.enqueue_any(entry).await }
    }

    /// Enqueue all values of type [T].
    #[inline]
    async fn enqueue_all<E: Into<T>>(
        &self,
        entries: impl IntoIterator<Item = E> + Clone,
    ) -> Result<(), QueueError> {
    ...
    #[inline]
    async fn dequeue_n(&self, n: usize) -> Result<Vec<T>, QueueError> {
