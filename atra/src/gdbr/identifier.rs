...
impl<'a, TF: TfAlgorithm, IDF: IdfAlgorithm> GdbrIdentifierCreationContext<TF, IDF> for InitHelper<'a, TF, IDF> {
    ...
    pub fn new_from_config<C: GdbrIdentifierCreationContext<TF, IDF> + SupportsStopwordsRegistry>(context: &C) -> Result<Option<Self>, SvmCreationError<IDF>> {
        if let Some(config) = context.gdbr_config() {
            ...
            let by_language = if let Some(ref others) = config.by_language {
                  others.iter().map( |(k, v)| {
                      match create_document_classifier(&v.identifier.svm, context.stopword_registry(), context.root()) {
                          Ok(value) => {
                              Ok(
...
mod test {
    ...
    use svm::{CsvProvider, CsvTrainModelEntry, read_train_data, train};
