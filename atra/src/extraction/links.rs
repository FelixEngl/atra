...
impl Hash for ExtractedLink {
    fn hash<H: Hasher>(&self, state: &mut H) {
        match self {
            ExtractedLink::Outgoing { url, .. } => url.hash(state),
            ExtractedLink::OnSeed { url, .. } => url.hash(state),
            ExtractedLink::Data { base, url, .. } => {
                base.hash(state);
                url.hash(state)
            }
...
impl PartialEq<Self> for ExtractedLink {
    fn eq(&self, other: &Self) -> bool {
        match self {
            ExtractedLink::Outgoing { url, .. } => match other {
                ExtractedLink::Outgoing { url: o_url, .. } => url == o_url,
                _ => false,
            },
            ExtractedLink::OnSeed { url, .. } => match other {
                ExtractedLink::OnSeed { url: o_url, .. } => url == o_url,
                _ => false,
            },
            ExtractedLink::Data { base, url, .. } => match other {
                ExtractedLink::Data {
                    base: o_base,
                    url: o_url,
                    ..
