...
fn _default_root_folder() -> Utf8PathBuf {
    "./atra_data".parse::<Utf8PathBuf>().unwrap()
}
...
fn _default_database_dir() -> Utf8PathBuf {
    "./rocksdb".parse::<Utf8PathBuf>().unwrap()
}
...
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct Files {
    /// Path to the queue file (if one is needed)
...
fn _default_queue_file() -> Utf8PathBuf {
    "./queue.tmp".parse::<Utf8PathBuf>().unwrap()
}
fn _default_blacklist_file() -> Utf8PathBuf {
    "./blacklist.txt".parse::<Utf8PathBuf>().unwrap()
}
