...
impl CachedRobots {
    #[allow(dead_code)]
    pub fn map<R, F>(&self, on_has_robot: F) -> Option<R> where F: FnOnce(&Robot) -> R {
        match self {
    ...
    #[allow(dead_code)]
    pub fn map_or<R, F>(&self, default: R, on_has_robot: F) -> R where F: FnOnce(&Robot) -> R {
        match self {
    ...
    #[allow(dead_code)]
    pub fn map_or_else<R, D, F>(&self, default: D, on_has_robot: F) -> R where
        D: FnOnce() -> R,
        F: FnOnce(&Robot) -> R {
        match self {
            CachedRobots::HasRobots{robot, ..} => { on_has_robot(robot)}
    ...
    pub fn delay(&self) -> Option<Duration> {
        self.map_or(None, |it| it.delay.map(|seconds| (seconds as f64).seconds()))
    }
