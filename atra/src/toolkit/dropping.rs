...
#[repr(transparent)]
pub struct DropNotifyer<T> where T: DropNotifyerEvent {
    sender: Sender<T>
}

impl<T> DropNotifyer<T> where T: DropNotifyerEvent + Default {
    #[inline]
...
impl<T> DropNotifyer<T> where T: DropNotifyerEvent {
...
impl<T> From<Sender<T>> for DropNotifyer<T> where T: DropNotifyerEvent {
    #[inline]
...
impl<T> Drop for DropNotifyer<T> where T: DropNotifyerEvent {
    fn drop(&mut self) {
