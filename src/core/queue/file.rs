...
use crate::core::queue::{AgingQueueElement, QueueError};
use crate::core::queue::traits::RawAgingQueue;
...
impl RawAgingQueue for RawAgingQueueFile  {
    async unsafe fn enqueue_any<E: AgingQueueElement + Serialize + Debug>(&self, mut entry: E) -> Result<(), QueueError> {
    ...
    async unsafe fn dequeue_any_n<E: AgingQueueElement + DeserializeOwned + Debug>(&self, n: usize) -> Result<Vec<E>, QueueError> {
        ...
        lock.iter()
            .take(n)
            .map(|value| {
                match bincode::deserialize(value.as_ref()) {
                    Ok(value) => {
                        Ok(value)
                    }
                    Err(err) => {
                        Err(QueueError::EncodingError(err))
                    }
                }
            })
            .collect()
...
impl Default for RawAgingQueueFile {
    fn default() -> Self {
        ...
        Self {
            queue: Arc::new(Mutex::new(QueueFile::open(temp_queue_file.as_path()).unwrap())),
            broadcast: tokio::sync::broadcast::Sender::new(1)
        }
    }
}
