...
use thiserror::Error;
use crate::core::link_state::{LinkState, LinkStateType};
...
macro_rules! declare_column_families {
    ($($self:ident.$db:ident => $name: ident($imported_name: ident))+) => {
        $(
            const $imported_name: &'static str = $crate::core::rocksdb_ext::$imported_name;
    ...
    ($db: ident: [$($handle_name: expr => (if test $init: ident else $message: literal))+]) => {
        $(
            if $db.cf_handle($handle_name).is_none() {
                if cfg!(test) {
                    $db.create_cf($handle_name, &$crate::core::rocksdb_ext::$init()).expect(
