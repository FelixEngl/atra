...
pub trait SupportsSimpleLineReader {
    ...
    fn to_simple_line_reader(self) -> SimpleLinesReader<Self> where Self: BufRead + Sized;
}
...
impl<T: AsRef<[u8]>> SimpleLinesReader<BufReader<Cursor<T>>> {
    #[cfg(test)] pub fn with_cursor(value: T) -> Self {
                                        ...
                                        Ok(mut next) => {
                                            ...
                                            if next.ends_with('\\') {
                                                ...
                                                if needs_processing || !next.ends_with('\\'){
                                                    collector.push('\n');
                                                    collector.push_str(&next[(starts_with_ignored_comment as usize)..]);
                                                    continue;
                                                }
                                            }
                                            collector.push('\n');
...
impl<I, T> SupportsSimpleLineMapper for I where I: Iterator<Item=T>, T: AsRef<str> {
...
mod tests {
    ...
    use crate::core::io::simple_line::{SupportsSimpleLineMapper, SimpleLinesReader};

    #[test]
    pub fn can_serialize_and_deserialize(){
        let data = vec!["Hallo Welt\nHier bin ich!", "# Also so kann \\ das nichts werden!\\", "Dieser satz\nmacht keinen\\\n# sinn!!!", "# Aber\nhallo!\n#wie viel wollen wir\ntun\n\noder so?"];
        // println!("{}", data.into_iter().to_simple_line_iter().join("\n"));
        let dat = data.clone().into_iter().to_simple_lines().join("\n");
        println!("{dat}");
        let result = SimpleLinesReader::with_cursor(dat).collect::<Result<Vec<_>,_>>().unwrap();
