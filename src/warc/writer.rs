...
use thiserror::Error;
use ubyte::ByteUnit;
use crate::warc::header::{WarcHeader, WarcHeaderWriteError};
...
impl<W: Write> WarcWriter<W> {
    ...
    #[allow(dead_code)] pub fn corrupted(&self) -> bool {
        self.corrupt
    }

    /// Sets the corruption flag to [new_corrupt].
    /// May cause the production of illegal WARC archives.
    #[allow(dead_code)] pub unsafe fn set_corrupt(&mut self, new_corrupt: bool) -> bool {
        std::mem::replace(&mut self.corrupt, new_corrupt)
    }

    /// Sets the state to [new_state].
    /// May cause the production of illegal WARC archives.
    #[allow(dead_code)] pub unsafe fn set_state(&mut self, new_state: State) -> State {
