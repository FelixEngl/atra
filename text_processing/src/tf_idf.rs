...
use crate::corpus::CorpusDocumentStatistics;

#[allow(dead_code)]
pub mod defaults {
    use crate::tf_idf::{Idf, Tf, TfIdf};
...
mod test {
    ...
    fn create_pseudo_random_statistics<R: SampleRange<usize>>(
        seed: u64,
        doc_range: R,
    ) -> CorpusStatisticsCollector<String> {
        let mut statistics = CorpusStatisticsCollector::default();

        statistics.add(
            lipsum_words_with_rng(rand::rngs::StdRng::seed_from_u64(seed), 30)
                .unicode_words()
        ...
        for _ in 0..random.gen_range(doc_range).saturating_sub(1) {
            ...
            let doc = lipsum_words_with_rng(random.clone(), 30)
                .unicode_words()
                .map(|value| value.to_string())
                .collect_vec();
            statistics.add(doc);
    ...
    fn tf_idf_works() {
        ...
        println!("{statistics}");
        let vectorizer = statistics
            .provide_vectorizer(super::defaults::RAW_INVERSE_SMOOTH)
            .unwrap();
        println!("\n----\n{vectorizer:?}");
        let doc_known = lipsum_words_with_rng(rand::rngs::StdRng::seed_from_u64(123456), 30)
            .unicode_words()
            .map(|value| value.to_string())
            .collect_vec();
    ...
    fn simple_tf_idf() {
        ...
        let doc3 = "i am going to watch the season premiere"
            .unicode_words()
            .collect_vec();
        ...
        println!("{statistics}");
        let vectorizer = statistics
            .provide_vectorizer(super::defaults::TERM_FREQUENCY_INVERSE)
            .unwrap();
        let doc_test = "bro it is going to rain today"
            .unicode_words()
            .collect_vec();
        ...
        fn test_vectorized(vectorized: VectorizedDocument<impl AsRef<str> + Debug + Display>) {
            ...
            for TfIdfVectorEntry(word, value) in vectorized.iter() {
                match word.as_ref() {
                    "to" | "today" => {
                        assert!(
                            (*value - 0.025155894150811604).abs() < f64::EPSILON,
                            "Failed for '{}', got {}",
                            word,
                            *value
                        )
                    ...
                    "it" | "is" | "rain" => {
                        assert!(
                            (*value - 0.06816017924566606).abs() < f64::EPSILON,
                            "Failed for '{}', got {}",
                            word,
                            *value
                        )
