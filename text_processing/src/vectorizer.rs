...
pub fn create_vectorizer<I: Iterator<Item=T>, T: AsRef<str>, Tf: TfAlgorithm, Idf: IdfAlgorithm>(train_data: I, tokenizer: &Tokenizer, tf_idf: TfIdf<Tf, Idf>) -> Result<DocumentVectorizer<String, Tf,Idf>, Idf::Error> {
...
impl<W> MappedDocumentVectorizer<W> where W: Hash + Eq {
    ...
    #[inline]
    fn vectorize_document<Tf: TfAlgorithm, D: IntoIterator<Item=W>>(&self, tf: &Tf, doc: D, normalized: bool) -> (usize, Vec<TfIdfVectorEntry<&W>>) {
        unsafe {
            self.vectorize_tf_document(tf.calculate_tf(doc), normalized)
        }
    }

    unsafe fn vectorize_tf_document(&self, doc: HashMap<W, f64>, normalized: bool) -> (usize, Vec<TfIdfVectorEntry<&W>>) {
