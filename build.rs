...
fn build_stop_word_library(object: &HashMap<String, JsonValue>) {
    ...
    for (k, v) in object.iter() {
        let lang = Language::from_639_1(k.as_str()).expect(format!("Why is {k} not an iso language?").as_str());
    ...
    let path = Path::new("src/features/tokenizing/stopwords/iso_stopwords.rs");
    let mut content = BufWriter::new(File::options().create(true).truncate(true).write(true).open(&path).unwrap());
    ...
    writeln!(&mut content, "pub fn iso_stopwords_for(lang: &isolang::Language) -> Option<&'static [&'static str]> {{").unwrap();
    ...
    for k in containers.keys() {
        ...
        writeln!(&mut content, "        isolang::Language::{} => Some(STOPWORDS_{}),", name, k.to_639_3().to_uppercase()).unwrap();
    }
    ...
    for (k, v) in containers {
        ...
        writeln!(&mut content, "const STOPWORDS_{}: &'static [&'static str] = &[", k.to_639_3().to_uppercase()).unwrap();
        for word in v {
